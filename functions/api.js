export default async function handler(request) {
  ...
  try {
    ...
    if (!startAddr || !endAddr) {
      return new Response(JSON.stringify({code:-1,msg:"缺少startAddr或endAddr"}), { headers });
    }
    ...
    if (data.status !== "1") {
      return new Response(JSON.stringify({code:-1,msg:"高德接口异常："+data.info}), { headers });
    }
    ...
    return new Response(JSON.stringify({code:-1,msg:"请求异常："+err.message}), { headers });
  }
}
